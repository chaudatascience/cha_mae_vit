...
class MAB(nn.Module):
    ...
    def __init__(self, dim_Q: int, dim_K: int, dim_V: int, num_heads: int, ln=False):
        super().__init__()
        self.dim_V = dim_V
    ...
    def forward(self, Q: Tensor, X: Tensor):
        ...
        Q_ = torch.cat(Q.split(dim_split, 2), 1)
        K_ = torch.cat(K.split(dim_split, 2), 1)
        V_ = torch.cat(V.split(dim_split, 2), 1)
